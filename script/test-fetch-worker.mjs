#!/usr/bin/env node
/**
 * Test script for fetch worker
 * Adds a test job to the sms-fetch queue
 */
...
async function testFetch() {
  console.log('=== Fetch Worker Test ===\n');

  try {
    // Add a test job
    console.log('1. Adding test fetch job...');
    const job = await addFetchJob({
      type: 'scheduled',
      delay: 100,
    });
    ...
    // Check queue stats
    console.log('\n4. Queue stats:');
    const stats = await fetchQueue.getJobCounts('waiting', 'active', 'completed', 'failed', 'delayed');
